...
void ofApp::draw() {
    ...
    if (bShowDebug) drawDebugOverlay();
}
...
void ofApp::processCommand(int key) {
    ...
    if (key == '1') {
        ...
        if (myTree.getMaxMutation() > 0.5f) spawn2DEffect(P_BLOOM);
        actionTaken = true;
    }
...
void ofApp::drawHUD() {
    ...
    string info = "DAY: " + ofToString(myTree.getDayCount()) + " / 50 | WEATHER: " + weather.getName();
    float infoW = mainFont.stringWidth(info) + 20;
    ofSetColor(0, 180);
    ofDrawRectangle(15 * scale, 15 * scale, infoW * scale, 30 * scale);
    ofSetColor(255);
    ofPushMatrix();
	ofScale(scale, scale);
    mainFont.drawString(info, 25, 35);
    ofPopMatrix();
    ofPopStyle();
    ...
    if (bGameEnded) {
        ...
        string endMsg = "GROWTH COMPLETED: [" + finalTitle + "]";
        mainFont.drawString(endMsg, ofGetWidth() / 2 - mainFont.stringWidth(endMsg) / 2 * scale, ofGetHeight() / 2);
    }
}
